...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Entry(frm_cfg, textvariable=self.var_date, width=14).grid(row=0, column=1, sticky="w", padx=4)
        ...
        ttk.Entry(frm_cfg, textvariable=self.var_uid, width=20).grid(row=1, column=1, sticky="w", padx=4)
        ...
        ttk.Entry(frm_cfg, textvariable=self.var_pw, width=20, show="*").grid(row=2, column=1, sticky="w", padx=4)

        # 利用目的
        ttk.Label(frm_cfg, text="利用目的（スポーツ名）").grid(row=3, column=0, sticky="w")
        self.var_purpose = tk.StringVar(value=s.get("purpose", "バレーボール"))
        ttk.Entry(frm_cfg, textvariable=self.var_purpose, width=20).grid(row=3, column=1, sticky="w", padx=4)

        # 利用人数
        ttk.Label(frm_cfg, text="利用人数").grid(row=4, column=0, sticky="w")
        self.var_people = tk.IntVar(value=s.get("num_people", 16))
        ttk.Spinbox(frm_cfg, textvariable=self.var_people, from_=1, to=100, width=6).grid(row=4, column=1, sticky="w", padx=4)
        ...
        ttk.Entry(frm_cfg, textvariable=self.var_discord, width=60).grid(row=5, column=1, sticky="w", padx=4)

        # 監視間隔
        ttk.Label(frm_cfg, text="監視間隔 (秒)").grid(row=6, column=0, sticky="w")
        self.var_interval = tk.IntVar(value=s.get("interval_sec", 5))
        ttk.Spinbox(frm_cfg, textvariable=self.var_interval, from_=5, to=60, width=6).grid(row=6, column=1, sticky="w", padx=4)
